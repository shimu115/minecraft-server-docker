...
第一阶段：Backend 后端重构
    ↓
第二阶段：Web UI 重构（重构方案暂定）
    ↓
第三阶段：业务逻辑深化 + 权限体系 + 文件管理
```
...
对 Spring Boot 后端进行全面重构，提升代码质量与工程规范度。

主要内容：

* 按照开发规范审查并重构现有代码（实体对象、异常处理、参数校验、日志、错误码等）
* 统一代码风格，消除重复逻辑，提取公共模块
* 梳理 Service 层职责边界，确保分层清晰
* 完善单元测试与集成测试覆盖
* 数据库表结构审查与索引优化
* API 接口规范对齐（URI、请求/响应格式、错误码）

---

### 第二阶段：Web UI 重构 *（重构方案暂定）*

对 Vue 3 前端进行全面重构，提升用户体验与代码可维护性。

> ⚠ **第二阶段重构方案尚未最终确定，以下为暂定方向。**

暂定方向：

* 前端架构重新评估（组件拆解、目录结构、状态管理）
* 组件库升级与统一
* 页面交互体验优化（加载态、空状态、错误态）
* 移动端适配方案
* 主题与样式系统整理
...
在前两阶段重构完成的基础上，建立完整的三层权限模型（角色边界 → 路径级 ACL → 资源保护兜底），
